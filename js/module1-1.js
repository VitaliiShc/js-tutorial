...
// // ❗️❗️❗️ SAY HELLO
// // Об'явлення змфнної (const, let)
// const helloMsg = 'Hello World';
// const wellcomeText = 'JavaScript is awesome';
// // Вивід повідомлення брацзера у вигляді модального вікна
// alert(helloMsg);
...
// //  ❗️❗️❗️ VARIABLES
// // Змінна, яку потрібно одразу ініціалізовати та не можана змінінти значення потім. Застосовується загально. Це нибито 'константа' (const).
// // const a; // error
// const a = 7;
// // a = 8; // error
// // Змінна, яку не потрибно ініціалізовувати значенням одразу при оголошенні та можно переівизначити. Застосовується коли потрібно зминяти змінну під час виконання скрипту, а також для використання для функції в локальної області видимості. Дослівно це означає "пусть" або "припустимо, що", : "Пусть x будет 9".
// let b; // underfined (невизначена)
// b = 9; //
// console.log(b); // 9
// // перевизначення b
...
// // Константа UPPER_SNAKE_CASE - використовується як вказівка, що цю змінну міняти не можна (як вказівка розробникам, що ця змінна типу зарезервована, та ії не слід зміняти)
...
// // ❗️❗️❗️ INPUT
// // alert() - виводить модальне вікно із кнопкою "OK" та блокує виконнання скрипта до натискання на кнопку
// alert('Hi!');
...
// // prompt() - віиволдить діалогове вікнео з рядком для введення та кнопкою "OK", блокує виконання скрипта доя натискання на кнопку, натискання на кнопку повертає введенне значення.
...
// // ❗️❗️❗️  ALGEBRA
// // Оператори - це знакі, яки використовуються для виконання дій (+, -, *, %, /, ...)
// // Операнди - значення, з якима виконуютьтся дії
// const x = 10;
// const y = 3;
...
// console.log('x = ', x);
// console.log('y = ', y);
// console.log(x + y); // додавання, але потрібно пам'ятати, що при роботі із різнимі типами операндів, якщо один з них string, математичний оператор '+' виконує конкатенацію, тобто складання рядків один до іншого, тому залежить від сітуаціі: для складання чисекл потрібно приводити іх до number, при необхідності конкатенації - приведення до number не потрібно.
...
// // ❗️ Поріваняння різних типів операндів
// // При не стпрогому порівнянні операедів різного типу, операнди приводяться до числа. При цьому (виключення):
// // null == null == undefined == undefined !== * (null та underfined дорівнюють один одному та не дорівнює нічому більше)
...
//  // Строги операторри  (=== і !== ) не виконують приведння типів та виконують порівняння операндів в тому вигляді як є:
...
// console.log(true == '1'); // true - виконується приведння числа в рядку number та правило, що true == 1, тобто 1 == 1
...
// console.log('x === z:', x === z); // false !!! строге порівняння: x-string не приводиться до number та залишається рядком, і рядок не рівен числу, тому це ложь
// console.log('x !== z:', x !== z); // true !!! строге порівняння: x-string не приводиться до number та залишається рядком, і рядок не рівен числу, тому не правда
// console.log('string == string:', x == y); // false порівняння за unicode, тому string '5' не рівно string '10'

// // ❗️❗️❗️ Числа. Number. NaN, isNaN

// // ❗️ Перетворення числа з рядку (якщо немає інших символів, окрім крапки, яка вважається розділення десятичної доробі) выконується за допомогою методу Number()
// const a = '5.1';
...
// // ❗️ Метод Number.isNaN() перевіряє, чи значення є числом, та повертає буль, але в зворотньму напрямку: "чи це НЕ число?": true - так, це НЕ ЧИСЛО; false - ні, це ЧИСЛО. Але функція сама не робить приведення, тому для виконання порівняння потрібно спочатку привести перевіряєме значення до number, а потім виконувати перевірку, інакше буде постійно повертати false. ПЕРЕВІРКУ НА NaN - ТІЛЬКИ ПІСЛЯ ПРИВЕДЕННЯ ДО NUMBER!:
...
// // при рахуванні в двоїчної системі дрібні числа можуть маюти похибку (особливості рахування в двоічної системі):
// console.log(0.1 + 0.2); // 0.30000000000000004
// console.log(0.1 + 0.2 === 0.3); // false

// // тому для таких випадків можна використовувати множення операндів на 10 (або 100, 1000 і т. і.) для приведення спочатку до цилих чисел, а потім ділення результату на той самий множник:
...
// // На заміну цього способу використовується метод toFixed() з вказанням, до якого знаку після коми потрібно відрізати. Виконується множення опреандів та потім ділення на потрібне значення для виправлення проблеми твоїчного обчісленнчя (так кажучи, все під капотом). Але метод toFixed повертає не number, а рядок, тому потім потрібно перетворити резуьтат на number.
...
// // ❗️ Math.floor(number) повертає найменше ціле число, менше ніж зазначенео - тобто,округляє в менший бік
...
// // олд-скульний метод возведення в ступень (з експонентою ( ** )):
...
// // ❗️ Math.random() повертає всевдо випадкове число (НЕ ВКЛЮЧНО! для включно - див інтернети)
...
// console.log(str1);
// // З конкатенацыєю порожнього рядка:
// const str2 = num + '';
// console.log(str2);
...
// // ❗️ Властивість length - Довжина рядка - повертає кількисть символів у рядку. Враховуються всі символи в томц числі і пробіли
...
// // ❗️ Метод indexOf() поваертає інтдекс позіції першого збігу підрядка, або -1 якщо збігу не знайдено. Регістр має значяення:
// const brandName1 = 'CanDy';
...
// // ❗️ Метод lastIndexOf() поваертає інтдекс позіції останнього збігу підрядка або -1 якщо збігу не знайдено. Регістр має значяення:
...
// // ❗️ Метод charAt() поверитає рядок з сімволом із рядка за вказаним індексом. Якщо зазнченний індекс виходить за рамки діапазону дліни рядка, то повертає порожній рядок. Аналог звернення до індексу зп тквадратнимі дужками
// const brandName1 = 'CanDy';
// console.log(brandName1.charAt(0)); // 'C'
// console.log(brandName1.charAt(7)); // ''
// console.log(brandName1[0]); // 'C'

// // ❗️ Метод includes() перевіряє, чи містится зазначений підрядок в рядку. Повертае true або false. Регістра має значення:
// const brandName1 = 'CanDy';
// console.log(brandName1.includes('Dy')); // true
// console.log(brandName1.includes('dy')); // false

// // ❗️ Метод endsWith() дозволяє визначити, чи завершуеться рядок зазначеними символами (підрядком). Повертае true або false. Регістр має значення:
...
// // ❗️ Метод startsWith() дозволяє визначити, чи починається рядок зазначеними символами (підрядком). Повертае true або false. Регістр має значення:
// const fileName = 'index.html';
...
// // ❗️ Метод replace() та replaceAll() повретає новий рядок, в якому віконано заміну збігів (першого (replace) або всіх (replaceAll)) на вказане значення. Регістр має значення:
// const someWord = 'banana';
...
// // ❗️ Метод slice (atartIndex, endIndex) дозволяє створити копію частини рядка (або всього) від startIndex (включно) до endIndex (не включно). Не змінює орігінал.
...
// // ❗️ Метод trim() видалє прибіли на початку та кінці строки, та повертає нову строку без початкових та кінцевих пробілів.
// const sameText = '   Text   ';
...
// // (falsy) значення, що перетворюється на false у логічносму перетворенні при приведенні до буля:
// // 0
// // NaN
// // null
// // undefined
...
// // ❗️ Логічне 'І' (and) - &&
// // повертає перший false, або останній true (толбто, зупинається на false, та повертає його значення; якщо всы операнди були приведені до true - полвертається останне значення).

// // ❗️ Логічне 'АБО' (or) - ||
// // повертає перший true, або останній false (тобто, зупеняється на true, та повертає його значення; якщо всі операнди привдені до false, повертає останнє значення)
// console.log(null || (2 && 3) || 4); // null || 3 (при && повертається 3 - тобто останне true) || 4 --> 3 || 4 (повертає перще true) --> 3

// // При використанні в одному вираженні && та || - && має приоритетність!

// // ❗️ Логічне 'НЕ' (not) - !
// // Інвертує false в true або true в false. Унарний оператор - виконує операцію над одним операндом. Використовується для роботи з розгалуженнями
...
// // ❗️ Оператор нудевого злиття - ??
// // Підрозділ логічного оператора &&. Якщо лівий операнд null або undefined - повертає правий операнд, в інших випадках повертає лівий опернд
